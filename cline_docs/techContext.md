...
- MCP server (internal_chat_mcp)
- Cursor IDE integration

## Development Setup
- Activate venv before installing or running tools.
- Install internal_chat_mcp in editable mode (`pip install -e .`).
- Configure `.cursor/mcp.json` with absolute paths for MCP server.

## Technical Constraints
- **Project path must not contain spaces or parentheses.**
  - MCP server process launcher in Cursor cannot handle spaces, leading to ENOENT errors and failed tool startup.
  - Always use a simple, space-free path for the project and venv.