...
- Uses PostgreSQL for task queue management
- ProcessingTask model tracks job status and progress
- Simple command-line execution for processing
- Same processing logic as web interface
- Error handling and logging built-in

## Task Management
- Tasks created via admin interface
- Status tracking: pending, processing, completed, failed
- Progress monitoring via counters
- Error details stored in JSONB field
- Metadata storage for task context

## Processing Flow
1. Task created in admin interface
2. Command-line processor picks up pending tasks
3. Each transaction processed with appropriate agent
4. Results stored in transaction records
5. Task status and progress updated
6. Errors logged and tracked

## Monitoring
- Admin interface for task status
- Progress tracking via counters
- Error details accessible in admin
- Logging for debugging
- Simple command-line interface

## Error Handling
- Transaction-level error tracking
- Task-level error logging
