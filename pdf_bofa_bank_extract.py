...
Usage:
    python pdf_bofa_bank_extract.py
"""
import pdfplumber
import pandas as pd
import re
...
OUTPUT_PATH_CSV = "ConsolidatedReports/BofA_bank.csv"  # need to update the AI_category to applie different filter for BofA
OUTPUT_PATH_XLSX = "ConsolidatedReports/BofA_bank.xlsx"
...
# TEST FUNCTIONS
pdf_path = "SourceStatements/BOA_Bank/eStmt_2022-01-14.pdf"
...
print(f"TEST 1 (page count): {get_page_count(pdf_path)}")
...
print(f"TEST 2 (extract by page number 3): {extract_text_by_page(pdf_path,3)}")
...
print(f"TEST 3 (check encryption using pdfplumber): {check_encryption(pdf_path)}")
...
# Extract lines from the checks section
print(f"TEST 4 (check sectin lines): {extract_checks_section(pdf_path)}")


# Display the first few lines for inspection
# print(checks_section_lines[:10])  # Show the first 10 lines
...
# deposits_df, withdrawals_df, checks_df = parse_transactions(text_list)
# file_paths = save_to_files(
#     deposits_df, withdrawals_df, checks_df, "bank_statement_transactions"
# )
...
# Run the script
combined_transactions_df = main()
combined_transactions_df.head()  # Display the first few rows for verification
