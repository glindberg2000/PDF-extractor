...
import pytest
from pdf_extractor_web.tools.search_tool.search_standalone import (
    search_web,
    SearchResult,
    SafeSearchLevel,
    SearchToolException,
...
def test_custom_language():
    ...
    try:
        results = search_web(query="bonjour monde", language="fr-FR", num_results=3)
...
def test_safe_search_levels():
    ...
    for level in SafeSearchLevel:
        try:
            results = search_web(query="cute cats", safesearch=level, num_results=1)
...
def test_custom_engines():
    ...
    try:
        results = search_web(
            query="latest news", engines=["google", "bing", "duckduckgo"], num_results=5
        )
